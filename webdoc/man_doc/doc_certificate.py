...
def _center(doc, txt, bold=False, spacing_before_pt=0, spacing_after_pt=0):
...
def _left(doc, txt, spacing_before_pt=0, spacing_after_pt=0):
    p = doc.add_paragraph(txt)
...
def _right(doc, txt, spacing_before_pt=0, spacing_after_pt=0, indent_spaces=2):
    p = doc.add_paragraph(txt)
...
def _hanging_label_paragraph(doc, label, text, label_width_cm=3.2, spacing_after_pt=0):
    """
...
def _hanging_continuation(doc, text, label_width_cm=3.2, spacing_after_pt=0):
    """
...
def doc_certificate(topic, author1, author2, dean, chairman, committee1, committee2):
    ...
    _center(doc, "ใบรับรองปริญญานิพนธ์", bold=True, spacing_after_pt=2)
    _center(doc, "คณะเทคโนโลยีและการจัดการอุตสาหกรรม", bold=True, spacing_after_pt=2)
    _center(doc, "มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าพระนครเหนือ", bold=True, spacing_after_pt=2)

    # ---------- เรื่อง / โดย (Hanging Indent) ----------
    LABEL_W = 0.9
    _hanging_label_paragraph(doc, "เรื่อง", topic or "", label_width_cm=LABEL_W)
    _hanging_label_paragraph(doc, "โดย", author1 or "", label_width_cm=LABEL_W)
    if (author2 or "").strip():
        _hanging_continuation(doc, author2, label_width_cm=LABEL_W)
    doc.add_paragraph()
    # ---------- ข้อความหลักใต้เรื่อง ----------
    _hanging_continuation(doc, "ได้รับอนุมัติให้นับเป็นส่วนหนึ่งของการศึกษาตาม", label_width_cm=LABEL_W)
    _hanging_continuation(doc, "หลักสูตรอุตสาหกรรมศาสตรบัณฑิต สาขาวิชาเทคโนโลยีสารสนเทศ",
